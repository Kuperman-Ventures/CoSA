...
function CalendarEventBlock({ ev, isPersonal, isUntaggedCosa, tag, onDelete, onTagClick, onEdit }) {
  ...
  return (
    <div
      className={`absolute left-0.5 right-0.5 rounded px-1 py-0.5 text-[10px] overflow-hidden group cursor-pointer
        ${isUntaggedCosa
          ? 'border border-dashed border-amber-400 bg-amber-50 hover:bg-amber-100'
          : needsTag
            ? 'border border-dashed border-slate-300 bg-slate-50 hover:bg-slate-100'
    ...
    >
      <div className="flex items-start justify-between gap-0.5">
        ...
        {/* Tag icon still shows on hover as a visual hint */}
        <div className="flex shrink-0 gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {(isPersonal || isUntaggedCosa) && (
            <span className="rounded p-0.5" title={isUntaggedCosa ? 'Tag this CoSA Calendar event' : tag ? 'Change tag' : 'Tag to track'}>
              <Tag size={9} />
            </span>
          )}
        </div>
