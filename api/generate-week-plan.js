export default async function handler(req) {
  ...
  // Slim the library — only send what the AI needs, skip heavy subtask text
...
- Deferred items must be placed first in the appropriate day and block.
- If KPI targets were missed, weight toward tasks that drive those KPIs.
...
Each task: {"templateId":"string","name":"string","track":"string","timeBlock":"string","estimateMinutes":number}`
...
Task library: ${JSON.stringify(slimLibrary)}`
  ...
  try {
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: data.error?.message ?? 'Anthropic error' }), { status: 500 })
    }
